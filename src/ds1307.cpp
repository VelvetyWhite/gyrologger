...
{
  i2c_write_blocking(m_i2c, DS1307_ID, &addr, 1, true);
  busy_wait_us(1);
  i2c_read_blocking(m_i2c, DS1307_ID, buf, len, false);
  busy_wait_us(1);
...
uint8_t MD_DS1307::writeDevice(uint8_t addr, uint8_t* buf, uint8_t len)
{
  ...
  len++;
  i2c_write_blocking(m_i2c, DS1307_ID, aux, len, false);
  busy_wait_us(1);
...
// Class functions
MD_DS1307::MD_DS1307(i2c_inst_t *i2c, int sda, int scl)
{
  m_i2c = i2c;
  m_sda = sda;
  m_scl = scl;
